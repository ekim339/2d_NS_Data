...
class Dataset(data.Dataset):
    def __init__(self, h5_path, use_double=False):
        self.dtype = torch.float64 if use_double else torch.float32
        
        # Load ALL data into memory at initialization
        with h5py.File(h5_path, 'r') as f:
            # Load as numpy arrays: (20, 1500, 64, 64)
            w_data = f['w'][:]
            u_data = f['u'][:]
            v_data = f['v'][:]
        
        # Stack to (20, 1500, 3, 64, 64)
        self.data = np.stack([w_data, u_data, v_data], axis=2)
        
        self.num_trajectories = self.data.shape[0]  # 20
        self.num_timesteps = self.data.shape[1]     # 1500
        self.total_samples = self.num_trajectories * self.num_timesteps
            
    def __len__(self):
        return self.total_samples
    
    def __getitem__(self, index):
        # Map flat index to (trajectory, timestep)
        traj_idx = index // self.num_timesteps
        time_idx = index % self.num_timesteps
        
        # Access pre-loaded data (FAST!)
        data = self.data[traj_idx, time_idx]  # (3, 64, 64)
